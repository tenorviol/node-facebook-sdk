...
function application(config) {
  return function createFacebook(req, res, next) {
...
// TODO : access token testing/support
// TODO : persistent methods should use callbacks
// TODO : rename to FacebookApiError and extend Error?
 ...
 */
var FacebookApiError = function (result) {
...
function Facebook(config) {
  if (!(this instanceof Facebook)) {
    var err = new Error('Facebook middleware is deprecated. Use facebook.application');
...
//  protected accessToken = null;
  ...
  _getUserAccessToken : function () {
    ...
    if (code && code != this._getPersistentData('code')) {
      var access_token = this._getAccessTokenFromCode(code);
      if (access_token) {
  ...
  _getRequest : function (key) {
    ...
    if (this.request.body) {
      return this.request.body[key] || null;
    }
    var parse = _url.parse(this.request.url, true);
    return parse.query[key] || null;
  },
  ...
  _getUserFromAvailableData : function () {
    ...
    //if (access_token &&
    //    access_token != this._getApplicationAccessToken() &&
    //    !(user && persisted_access_token == access_token)) {
    //  user = this._getUserFromAccessToken();
    //  if (user) {
    //    this._setPersistentData('user_id', user);
    //  } else {
    //    this._clearAllPersistentData();
...
//  }
    ...
    this._oauthRequest(
      ...
      function (err, result) {
        ...
        if (result && result.error_code) {
          callback(new FacebookApiError(result));
        } else {
    ...
    this._oauthRequest(
      ...
      function (err, result) {
        ...
        if (result && result.error) {
          callback(that._processAPIError(result));
        } else {
          callback(null, result);
        }
      }
  ...
  _makeRequest : function (url, params, callback) {
    ...
    // TODO : look into using native socket timeout method?
    if (this.timeout > 0) {
      timeout = setTimeout(function () {
        request.abort();
        var e = new FacebookApiError({
  ...
  _parseSignedRequest : function (signed_request) {
    ...
    // decode the data
    var sig = _base64UrlDecode(encoded_sig);
    var data = JSON.parse(_base64UrlDecode(payload));
   ...
   */
  _processAPIError : function (result) {
    ...
    switch (err.getType()) {
      ...
      case 'invalid_token':
        ...
        if ((message.indexOf('Error validating access token') >= 0) ||
            (message.indexOf('Invalid OAuth access token') >= 0)) {
...
//  abstract protected function _clearAllPersistentData();
  ...
  _setPersistentData : function (key, value) {
    ...
    if (!this._kSupportedKeys[key]) {
      this._errorLog('Unsupported key passed to _setPersistentData.');
      return;
    }
  ...
  _getPersistentData : function (key, fallback) {
    ...
    if (!this._kSupportedKeys[key]) {
      this._errorLog('Unsupported key passed to _getPersistentData.');
      return fallback;
    }
